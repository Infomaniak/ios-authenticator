...
struct ContactSupportSettingsView: View {
    ...
    @State private var authorisationCode: String?
    ...
    var body: some View {
        List {
            Section {
                ...
                if let authorisationCode {
                    InfoCell(AuthenticatorResourcesStrings.authorizationCodeTitle, value: authorisationCode)
                }
            } header: {
        ...
        .task {
            ...
            authorisationCode = getAuthorisationCode(for: selectedAccount) // TODO: Fetch real code time from API
        }
    }

    private func getAuthorisationCode(for account: UIAccount?) -> String? {
        return "12345"
    }
}
